...
def extract_list_of_cell_lines(lane_design_round_1_file, lane_design_round_2_file, lane_design_round_3_file):
    ...
    # Quick sanity check. Check to make sure we got 15 unique cell lines:
    if len(cell_lines) != 15:
...
def create_sample_id_to_fastqs_mapping(mapping, sample_id, cell_line, time_step, fastq_round_1_input_dir, fastq_round_2_input_dir, fastq_round_3_input_dir, lane_design_round_1_file, lane_design_round_2_file, lane_design_round_3_file):
    # Extract array of tuples from fastq_round_1_input_dir where each tuple represents (lane,sample_num) corresponding to this sample_id
...
lane_design_round_1_file = sys.argv[5]  # File containing information for mapping from sequencer name to cellLine_timeStep (sample) notation for the first sequencing round
lane_design_round_2_file = sys.argv[6]  # File containing information for mapping from sequencer name to cellLine_timeStep (sample) notation for the second sequencing round
lane_design_round_3_file = sys.argv[7]  # File containing information for mapping from sequencer name to cellLine_timeStep (sample) notation for the third sequencing round
fastq_dir = sys.argv[8]  # Output file
...
cell_lines = extract_list_of_cell_lines(lane_design_round_1_file, lane_design_round_2_file, lane_design_round_3_file)
...
for cell_line in cell_lines.keys():
    for time_step in range(16):
        ...
        if cell_line == '19209' and time_step == 13:  # This time point is contaminated
            continue
        if cell_line == '19159' and time_step == 7:  # This point just has very few reads mapped (so we ignore)
            continue
        sample_id = cell_line + '_' + str(time_step)
        mapping[sample_id] = []  # Initialize sampleID in mapping
        mapping = create_sample_id_to_fastqs_mapping(mapping, sample_id, cell_line, time_step, fastq_round_1_input_dir, fastq_round_2_input_dir, fastq_round_3_input_dir, lane_design_round_1_file, lane_design_round_2_file, lane_design_round_3_file)
